...
class Saml2Controller extends Controller
{
    ...
    {
	//dd(auth('api')->user());
        $returnTo = 'https://marketcheck-api.mzzo.com/'; //$request->query('returnTo');
        $sessionIndex = '_1d1e61e8-bfbf-4223-871d-86da3ed3b366';//$request->query('sessionIndex');
        $nameId = 'omar.curvelo@externos-cl.cencosud.com'; //$request->query('nameId');
	//dd($returnTo, $sessionIndex, $nameId);
	...
	$newLogin = new VerifyLoginToken();
        $newLogin->hast_email  = request()->cookie('fulanodetalpalback');
        $newLogin->app_token  =  request()->cookie('mzzo_session');
        $newLogin->token_user  = request()->cookie('fulanodetalpalbackTK');
        $newLogin->type_register  = 0;
        $newLogin->save();
	//Cookie::forget('fulanodelt');
	//Cookie::forget('fulanodetalpalbackTK');
	//Cookie::queue(Cookie::forget('fulanodelt'));
	//Cookie::queue(Cookie::forget('fulanodetalpalbackTK'));
